...
function getsmilies () {
  ...
  $b .= ' x.document.write("<a href=\"javascript:window.close();\">'.$lang['close'].'"); x.document.close(); }</script>';
  if ($i>100) { $a .= '<a href="javascript:moreSmilies();">'.$lang['more'].''; }
  $a = $b.$a;
  return ($a);
}
...
function icmail ($mail, $bet, $txt, $from = '', $html = false) {
	...
	if ($allgAr['mail_smtp']) { //SMTP Versand
		...
		if (empty($smtpser)) {
			echo '<div class="alert alert-danger" role="alert"><i class="fa fa-exclamation-triangle" aria-hidden="true"></i> Mailversand muss konfiguriert werden!</div>';
		} else {
...
function get_antispam($m, $t, $nopictures = false)
{
    ...
    $img = '<img width="' . $imagewidth . '" height="' . $imageheight . '" src="include/includes/captcha/captchaimg.php?id='
        ...
        . $seperator . '<input style="margin:3px;" class="captcha_code " name="captcha_code" type="text" maxlength="5" size="8" title="Geben Sie die Zeichen aus dem Bild ein">'
        . '<input type="hidden" name="captcha_id" value="' . $id .  '" />';
        ;

    if ($t == 1) {
        $img = '<tr><td><b>Antispam </b></td><td class="Cnorm">' . $img . '</td></tr>';
    } elseif ($t > 10) {
