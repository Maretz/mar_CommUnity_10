...
function icUpload () {
      ...
      if (empty($name)) {
        return ('<div class="alert alert-danger">keinen Namen angegeben.<br><br><a class="btn btn-default" href="javascript:history.back()">&laquo; zur&uuml;ck</a></div>');
      }

      if (empty($desc) or empty($descl)) {
        return ('<div class="alert alert-danger">Kein langer oder/und kein kurzer Text angegeben.<br><br><a class="btn btn-default" href="javascript:history.back()">&laquo; zur&uuml;ck</a></div>');
      }

      if (empty($url) AND empty($_FILES['file']['name'])) {
        return ('<div class="alert alert-danger">Keine Datei oder Link angegeben.<br><br><a class="btn btn-default" href="javascript:history.back()">&laquo; zur&uuml;ck</a></div>');
      }
        ...
        $fende = strtolower($fende);
          ...
          return ('<div class="alert alert-info">Die Datei darf NICHT gr&ouml;sser als 2 MBytes sein.<br><br><a class="btn btn-default" href="javascript:history.back()">&laquo; zur&uuml;ck</a></div>');
        }
        ...
        if (
          ...
          return ('<div class="alert alert-info">Die Datei darf nur die Endungen: .zip, .tar oder .rar haben.<br><br><a class="btn btn-default" href="javascript:history.back()">&laquo; zur&uuml;ck</a></div>');
        }
        ...
        if (file_exists( 'include/downs/downloads/user_upload/'.$fname ) ) {
          return ('<div class="alert alert-info">Die Datei existiert bereits und kann nicht &uuml;berschrieben werden.<br><br><a class="btn btn-default" href="javascript:history.back()">&laquo; zur&uuml;ck</a></div>');
        }
...
switch ( $menu->get(1) ) {
  default :
    ...
    if (@db_num_rows($erg) <> 1) {
      ...
      $design = new design ( $title , $hmenu );
      $design->header();
      echo '<div class="alert alert-danger">Der Download wurde nicht gefunden.<br><br><a class="btn btn-default" href="javascript:history.back()">&laquo; zur&uuml;ck</a></div>';
      $design->footer(1);
    }
  ...
  case 'upload' :
    if ( $allgAr['archiv_down_userupload'] == 1 AND loggedin() AND is_writeable ( 'include/downs/downloads/user_upload' ) ) {
      ...
      $re = icUpload();
	    if ($re === true) {
        echo '<div class="alert alert-success">Erfolgreich eingetragen! ... ein Moderator oder Admin dieser Seite wird den Download in n&auml;chster Zeit freischalten.<br><br><a class="btn btn-default" href="index.php?">&laquo; zur Startseite</a></div>';
      } else {
        echo $re;
      }
