...
if (count($far) <> $x OR $ch_name == false OR !chk_antispam('joinus')) {
  ...
  foreach($far as $v) {
		if ($x > 0 AND empty($_POST[$v])) {
			echo '<div class="alert alert-danger"><i class="fa fa-exclamation-triangle"></i> <b>missing:</b> '.$lang[$v].'</div>';
		}
		$tpl->set ($v, $$v);
	}
  ...
  $tpl->set('readonly', (loggedin()?' readonly': ''));
    ...
    $rules = '<legend>'.$lang['rules'].'</legend>';
    $rerg = db_query('SELECT zahl,titel,text FROM `prefix_rules` ORDER BY zahl');
    while ($rrow = db_fetch_row($rerg)) {
            $rules .= '<table width="100%" class="table  table-bordered">';
            $rules .= '<tr><td><b>&sect;'.$rrow[0].'. &nbsp; '.$rrow[1].'</b></td></tr>';
            $rules .= '<tr><td>'.bbcode($rrow[2]).'</td></tr>';
            $rules .= '</table><br>';
    }
