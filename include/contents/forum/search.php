...
$s = "DISTINCT b.id as fid, a.name as titel, a.id as id, d.name as author, c.time as time, c.erst as erst";
$q = "SELECT {SELECT}
...
$q2 = "SELECT DISTINCT b.id as fid, a.name as titel, a.id as id, MIN(c.id) AS firstnew, d.name as author, c.time as time, c.erst as erst
    FROM prefix_topics a
      LEFT JOIN prefix_forums b ON b.id = a.fid
      LEFT JOIN prefix_posts c ON c.tid = a.id
      LEFT JOIN prefix_user d ON c.erstid = d.id
...
while($r = db_fetch_assoc($q) ) {
    ...
    $r['avatar']  = (!empty($ergava) AND file_exists($ergava)) ? '<img class="pull-left showforumavatar" style="margin-bottom:20px;" src="'.$ergava.'" alt="Avatar" />' : '<img class="pull-left showforumavatar" src="include/images/avatars/wurstegal.jpg" style="margin-bottom:20px;" />';
    if ($menu->get(1) == 'aeit') {
        $r['author'] = '';
    } elseif ($such == 'aubt') {
        $r['author'] = $lang['from'].' '.$r['author'];
    } else {
        $r['author'] = 'Thema '.$lang['from'].' '.$r['author'];
