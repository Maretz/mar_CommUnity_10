...
if ((isset($_POST['del']) OR isset($_POST['shift']) OR isset($_POST['status'])) AND (empty($_POST['in']) OR (isset($_POST['in']) AND (!is_array($_POST['in']) OR count($_POST['in']) < 1)))) {
  ...
  echo '<form action="index.php?forum-editforum-'.$fid.'" method="POST" class="form-horizontal">';
  foreach ($_POST['in'] as $k => $v) {
    echo '<input type="hidden" name="in['.$k.']" value="'.$v.'" />';
  }
  if (isset($_POST['del'])) {
    echo '<input type="hidden" name="dely" value="yes" />';
    echo '<div class="alert alert-danger"><p>Sicher die ausgewahlten Themen loeschen?</p><p><input type="submit" class="btn btn-danger" value="'.$lang['yes'].'" name="del" /></p></div>';
  } elseif (isset($_POST['shift']) AND $_POST['nfid'] != 'cat') {
    echo '<input class="form-control" type="hidden" name="afid" value="'.$fid.'"><legend>neues Forum ausw&auml;hlen</legend><br>';
    echo '<select class="form-control" name="nfid">';
      ...
      forum_admin_selectcats(0,0,$fid);
		echo '</select><br /><input type="checkbox" name="alertautor" value="yes" /> Die Autoren &uuml;ber das verschieben informieren?<br /><br /><input class="btn btn-primary btn-xs" type="submit" value="'.$lang['shift'].'" name="shift">';
  }
