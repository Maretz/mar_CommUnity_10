...
$erg = db_query("SELECT pruef, DATE_FORMAT(von,'%Y-%m-%d') as von, DATE_FORMAT(bis,'%Y-%m-%d') as bis, betreff, prefix_user.name, uid, prefix_awaycal.id FROM prefix_awaycal LEFT JOIN prefix_user ON prefix_user.id = prefix_awaycal.uid ORDER BY id DESC");
while($r = db_fetch_assoc($erg)) {
...
if ($e == FALSE) { $ar = array('id'=>'','von'=>date('Y-m-d'),'bis'=>date('Y-m-d'),'betreff'=>''); }
$tpl->set_ar_out($ar,0);
$design->footer();
?>