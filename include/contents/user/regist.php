...
if ($allgAr['forum_regist'] == 0) {
    ...
    echo '<div class="alert alert-danger">Der Administrator hat festgelegt das man sich nicht registrieren kann</div>';
        ...
        if (empty($name) OR empty($email) OR $name != $xname OR $ch_name == false OR !$chkAntispam) {
            ...
            if (isset($_POST['register'])) {
                if (empty($name) OR empty($email)) {
                    $fehler = $lang['<div class="alert alert-danger">entereddataincomplete</div>'];
                } elseif ($name != $xname) {
                    $fehler = $lang['<div class="alert alert-danger">wrongnickname</div>'];
                } elseif ($ch_name == false) {
                    $fehler = $lang['<div class="alert alert-danger">namealreadyinuse</div>'];
                } elseif (!$chkAntispam) {
                    $fehler = $lang['<div class="alert alert-danger">wrongcaptchaentered</div>']; //antispamtxt
                }
            } else {
                $fehler = '';
            }
