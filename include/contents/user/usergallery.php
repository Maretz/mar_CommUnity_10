...
if (empty($uid)) {
  ...
  $x = '';
  if (loggedin()) {
    $x .= '<a class="btn btn-primary " href="index.php?user-usergallery-'.$_SESSION['authid'].'">zu meiner Gallery</a><br><br><legend><i class="fa fa-picture-o"></i> User Gallery</legend><br>';
  }
  ...
  while ($r = db_fetch_assoc($erg)) {
    ...
    $x .= '<table class="table table-bordered"><tr><td class="active"><a href="index.php?user-usergallery-'.$r['uid'].'">'.$r['uname'].'</a></td><td class="text-right">Anzahl Bilder: <span class="badge">'.$r['anz'].'</span></td></tr></table>';
...
# bilder abfragen
    ...
    if ( db_num_rows($erg) > 0 ) {
        ...
        if (loggedin() AND (is_siteadmin() OR $uid == $_SESSION['authid'])) {
          $row['besch'] .= ' <a href="index.php?user-usergallery-'.$uid.'-p'.$page.'-d'.$row['id'].'" rel="tooltip" title="Eintrag löschen"><i class="fa fa-times-circle"></i></a>';
        }
        $row['width'] = round( 100 / $img_per_line );
        if ( $i <> 0 AND ($i % $img_per_line ) == 0 ) { echo ''; }
